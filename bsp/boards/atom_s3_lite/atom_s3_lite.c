 ...
 *
 * M5Stack AtomS3 Lite (ESP32-S3). No display: bring-up wires only the single
 * WS2812 RGB LED (GPIO35) and the front button (GPIO41). The button has an
 * external pull-up on the board, so no internal pull is requested. bsp_init
 * starts the shared input task itself since there is no touch reader to do it.
 */
...
static const char *TAG = "atom_s3_lite";

#define ATOM_LED_GPIO      GPIO_NUM_35
#define ATOM_LED_COUNT     1
#define ATOM_BUTTON_GPIO   GPIO_NUM_41
