...
static esp_err_t draw_bitmap(bsp_display_t *self, bsp_rect_t rect, const void *data,
    ...
    /* MIPI fast path is the framebuffer flush; draw_bitmap is the fallback so we
     * only handle the un-rotated source. A rotated source would need a scratch
     * un-rotated copy, which no current caller exercises. */
 ...
 * starts the DPI video stream — the vendor sequence ends with DISPON. */
static esp_err_t send_init_sequence(esp_lcd_panel_io_handle_t io, bool rgb888) {
    esp_err_t err;

    if ((err = esp_lcd_panel_io_tx_param(io, LCD_CMD_SWRESET, NULL, 0)) != ESP_OK) return err;
    vTaskDelay(pdMS_TO_TICKS(20));

    if ((err = esp_lcd_panel_io_tx_param(io, LCD_CMD_SLPOUT, NULL, 0)) != ESP_OK) return err;
    vTaskDelay(pdMS_TO_TICKS(120));

    /* MADCTL = 0x00 → RGB element order (default landscape, no mirror/flip). */
    const uint8_t madctl = 0x00;
    if ((err = esp_lcd_panel_io_tx_param(io, LCD_CMD_MADCTL, &madctl, 1)) != ESP_OK) return err;

    /* COLMOD: 0x55 = 16bpp RGB565, 0x77 = 24bpp RGB888. */
    const uint8_t colmod = rgb888 ? 0x77 : 0x55;
    if ((err = esp_lcd_panel_io_tx_param(io, LCD_CMD_COLMOD, &colmod, 1)) != ESP_OK) return err;

    for (size_t i = 0; i < ili9881c_init_cmds_count; i++) {
        err = esp_lcd_panel_io_tx_param(io, ili9881c_init_cmds[i].cmd,
                                        ili9881c_init_cmds[i].data,
                                        ili9881c_init_cmds[i].data_bytes);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "init cmd 0x%02x failed: %s", ili9881c_init_cmds[i].cmd,
                     esp_err_to_name(err));
            return err;
        }
        if (ili9881c_init_cmds[i].delay_ms) {
            vTaskDelay(pdMS_TO_TICKS(ili9881c_init_cmds[i].delay_ms));
        }
    }
    return ESP_OK;
}
...
esp_err_t ili9881c_lcd_create(const ili9881c_config_t *config, bsp_display_t **out) {
    ...
    /* esp_lcd_new_panel_dpi's init starts the video stream. */
