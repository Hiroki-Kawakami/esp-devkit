...
/* While set, the driver's bsp_touch_t::poll invokes fn(handle, ctx, paired)
 * after each touch read (paired = a HotKnot pair event was latched this poll).
 * NULL restores plain touch. HotKnot installs its state machine here. */
typedef void (*gt911_session_step_fn)(gt911_handle_t handle, void *ctx, bool paired);
