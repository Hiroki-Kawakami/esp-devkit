 ...
 * optional backlight). The controller GRAM is 240x320; a smaller glass is
 * addressed through the column/row offsets. Pixels are RGB565; the glass wants
 * them big-endian, so draw_bitmap byte-swaps the host-native (little-endian)
 * source while streaming it over queued DMA.
 *
 ...
 * NULL when the board has no backlight control.
 */
