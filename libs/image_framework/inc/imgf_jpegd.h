 ...
 * int16 coefficients for the whole frame (w*h*2 bytes per component, ~3 MB for
 * a 1000x1000 4:2:0 picture) out of `alloc_caps`; the downscale factor does
 * not shrink it. They are allocated one block row at a time, so a fragmented
 * heap with the bytes free in total can still serve them. A truncated or damaged stream keeps the scans that arrived
