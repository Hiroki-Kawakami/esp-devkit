...
const uint8_t arrow_image_map[] = {
    // Palette: transparent, black.
    0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0xff,
    // 16x supersampled SVG, quantized like a 4bpp font mask before I1 output.
    ...
    0x0f, 0x00, 0x1c, 0x00, 0xf0,
    0x1f, 0x00, 0x1e, 0x00, 0xf8,
    0x1e, 0x00, 0x1f, 0x00, 0x78,
    0x3c, 0x00, 0x1f, 0x80, 0x3c,
    0x3c, 0x00, 0x0f, 0xc0, 0x3c,
    0x3c, 0x00, 0x07, 0xe0, 0x3c,
    0x78, 0x00, 0x03, 0xf0, 0x1e,
    0x78, 0x00, 0x01, 0xf8, 0x1e,
    0x78, 0x3f, 0xff, 0xfc, 0x1e,
    0x78, 0x3f, 0xff, 0xfc, 0x1e,
    0x78, 0x3f, 0xff, 0xfc, 0x1e,
    0x78, 0x3f, 0xff, 0xfc, 0x1e,
    0x78, 0x00, 0x01, 0xf8, 0x1e,
...
const lv_image_dsc_t arrow_image = {
    .header = {
    ...
    .data_size = sizeof(arrow_image_map),
    .data = arrow_image_map,
};
...
lv_obj_t *lv_list_create(lv_obj_t *parent, ListDataSource *data_source) {
    ...
    lv_obj_set_size(state->previous_button, 60, 60);
    lv_obj_align(state->previous_button, LV_ALIGN_LEFT_MID, 12, 0);
    lv_obj_t *previous_image = lv_image_create(state->previous_button);
    lv_image_set_src(previous_image, &arrow_image);
    lv_image_set_pivot(previous_image, 20, 20);
    lv_image_set_rotation(previous_image, 1800);
    lv_obj_center(previous_image);
    ...
    lv_obj_set_size(state->next_button, 60, 60);
    lv_obj_align(state->next_button, LV_ALIGN_RIGHT_MID, -12, 0);
    lv_obj_t *next_image = lv_image_create(state->next_button);
    lv_image_set_src(next_image, &arrow_image);
    lv_obj_center(next_image);
